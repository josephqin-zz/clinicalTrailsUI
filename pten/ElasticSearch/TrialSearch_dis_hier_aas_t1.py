...
from pten.ElasticSearch.esoperations_disease import ESOperations
from pten.ElasticSearch import config
# from weights_config import *
from pten.ElasticSearch import weights_config as wconf

from pten.ElasticSearch import outputs_for_front as outff
from pten.ElasticSearch import tree_build_out as tb
...
disease_multi_query_fields = [
                              "Purpose", "description", "Inclusion Criteria", "official_title", "brief_title",
                               "Conditions",
...
disease_multi_query_fields = ["Purpose.disease^" + wconf.w_purpose, "description.disease^" + wconf.w_des,
                              "Inclusion Criteria.disease^" + wconf.w_ic,
                              "official_title.disease^" + wconf.w_ot, "brief_title.disease^" + wconf.w_bt,
                              "Conditions.disease^" + wconf.w_con, "Conditions.disease_whitespace",
                              "Purpose.disease_whitespace^" + wconf.w_purpose,
                              "description.disease_whitespace^" + wconf.w_des,
                              "Inclusion Criteria.disease_whitespace^" + wconf.w_ic,
                              "official_title.disease_whitespace^" + wconf.w_ot,
                              "Purpose", "description", "Inclusion Criteria", "official_title", "brief_title",
                              "Conditions",
                              "Purpose.whitespace", "description.whitespace", "Inclusion Criteria.whitespace",
                              "official_title.whitespace", "brief_title.whitespace", "Conditions.whitespace"
...
gene_multi_query_fields = ["Purpose", "description", "Inclusion Criteria", "official_title", "brief_title",
                           ...
                           "Purpose.whitespace", "description.whitespace", "Inclusion Criteria.whitespace",
                           "official_title.whitespace", "brief_title.whitespace", "Conditions.whitespace"]
disease_multi_query_fields_2 = disease_multi_query_fields + ["Exclusion Criteria", "Exclusion Criteria.whitespace", "Exclusion Criteria.normal"]
...
def query_body(myargs):
    ...
    if myargs.stage is not None:
        ...
        # match documents with no stage mentioned
        body["query"]["bool"]["must"][-1]["bool"]["should"].append({
            "constant_score": {
                "filter": {
    ...
    if myargs.include_keywords is not None:
        search_factors.append('--include_keywords')
        search_factors.append(myargs.include_keywords)
        include_keywords = myargs.include_keywords
        match_operator = 'or'
        if 'AND' in myargs.include_keywords:
            match_operator = 'and'
            include_list = list(map(lambda x: x.strip(' '), include_keywords.split('AND')))
            include_keywords = ' '.join(include_list)
        body["query"]["bool"]["must"].append({
            "multi_match":
                {
                    "query": include_keywords,
                    "type": wconf.multi_match_type,
                    "boost": wconf.boost_must_value,
                    "fields": disease_multi_query_fields_2,
                    "operator": match_operator}})
    if myargs.exclude_keywords is not None:
        search_factors.append('--exclude_keywords')
        search_factors.append(myargs.exclude_keywords)
        match_operator = 'or'
        body["query"]["bool"]["must_not"].append({
            "multi_match":
                {
                    "query": myargs.exclude_keywords,
                    "type": wconf.multi_match_type,
                    "fields": disease_multi_query_fields_2,
                    "operator": match_operator}})        
...
def get_hits(response, aas, aas_hier=False):
    ...
    for hit in response['hits']['hits']:
        if aas_hier:
            ...
            hits.append({'id': hit['_id'], 'score': hit['_score'],'context': hit['_source']})
    return hits, count
...
def normal(args,index):
    ...
    res,count = outff.tool_only(response)
    return res,count
    """
