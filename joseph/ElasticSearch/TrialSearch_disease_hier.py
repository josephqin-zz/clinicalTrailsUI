...
from joseph.ElasticSearch import config as config
from joseph.ElasticSearch import weights_config as ws

from joseph.ElasticSearch  import tree_build_out as tb


def es_DiseaseOntology(index_name,parameters):
	es = ESOperations (index_name)

	p_disease=parameters['disease']
	p_gene=parameters['gene']
	p_aas = parameters['aas']
	p_end = None
	p_start = None
	p_slop = None
	p_age = parameters['age']
	p_results = 50
	p_gender = parameters['gender']
	p_stage = parameters['stage']
	p_grade = parameters['grade']
	...
	if p_disease is not None:
		...
		body["query"]["bool"]["must"].append ({
			"multi_match":
				{
					"query": p_disease,
					"type": ws.multi_match_type,
					"operator": ws.multi_match_operator,
					"boost": ws.boost_must_value,
					"fields": disease_multi_query_fields}})
		if ws.use_should:
	...
	while diseases:
		...
		for disease in diseases:
			...
			if level == 1:
				...
				update_print (l['nct'], response, hit_ids)
			else:
				new_count = update_print (l['nct'], response, hit_ids)
				count += new_count
